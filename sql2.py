...
candle_data = defaultdict(dict)

# --- Utilities ---
def get_candle_time1(epoch_ms):
    dt = datetime.fromtimestamp(epoch_ms / 1000)
    dt = dt.replace(second=0, microsecond=0)
    return dt.strftime("%Y-%m-%d %H:%M:%S")

def get_candle_time(epoch_ms):
    # Convert epoch to UTC datetime
    dt_utc = datetime.utcfromtimestamp(epoch_ms / 1000)
...
def store_to_db(token, candle_time, c):
    cur.execute("""
        INSERT OR REPLACE INTO candles (token, candle_time, open, high, low, close)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (token, candle_time, c['open'], c['high'], c['low'], c['close']))
    conn.commit()

def update_candle(token, price, epoch_ms):
    # print(epoch_ms, price)
    candle_time = get_candle_time(epoch_ms)

    # Flush old candle
    prev_times = list(candle_data[token].keys())
    if prev_times and candle_time != prev_times[-1]:
        prev_time = prev_times[-1]
        store_to_db(token, prev_time, candle_data[token][prev_time])
        del candle_data[token][prev_time]

    # Update or create current candle
    if candle_time not in candle_data[token]:
        candle_data[token][candle_time] = {
            "open": price,
            "high": price,
            "low": price,
            "close": price
        ...
        c = candle_data[token][candle_time]
...
def on_data(wsapp, message):
    ...
    try:
        ...
        if token and ltp:
            ...
            update_candle(token, ltp, epoch_ms)
